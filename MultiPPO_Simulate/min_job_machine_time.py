...
def min_job_mch(mch_time,job_time, mchsEndTimes, number_of_machines, dur, temp, first_col,mask_last,done,mask_mch):
    ...
    while True:
        ...
        min_job_time = np.where(job_time1 <= job_time.min())[0] # 找出具有最短结束时间的作业

        min_task = first_col[min_job_time] # 获取这个作业的第一个任务
        ...
        for i in min_task:
            mask[np.where(first_col == i)] = 0 # 标记那些已经选择的任务

        mask = mask_last + mask # 合并掩码

        if done:
            break
        elif np.all(mask) == True:
            job_time = np.delete(job_time, np.where(job_time == job_time.min())[0]) # 检查是否所有任务的掩码都被设置为 True（意味着没有可执行的任务了）
        else:
            break
